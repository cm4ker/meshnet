...
4. Разместить [privacy.html](../apps/web/public/privacy.html) на публичном HTTPS-адресе
   без авторизации и геоблокировок; проверить текст и контактные данные.
   В приложении уже есть локальная копия, но это не заменяет публичный URL в Console.
   Например, файл можно разместить вместе с веб-клиентом на принадлежащем вам сайте.
