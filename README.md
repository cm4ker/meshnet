...
Three sections: **Chats** (who you talk to), **Mesh** (who is around: people, repeaters, rooms and sensors, on a map and in a list) and **Radio** (this radio and the app). A phone shows one screen at a time over its tab, with a tab bar below; a swipe from the left edge, or Android's Back, goes back. A desktop shows the list, what was picked in it and a panel of its details side by side, with Ctrl+K to jump anywhere and keys for the rest (Alt+1–3 for the sections in a browser, Ctrl+1–3 in the desktop shell, Alt+↑/↓ between chats, Ctrl+I for the panel, Esc to close it). A long press on a phone, or a right click, opens a row's or a message's menu. Every action that transmits carries an antenna mark.
...
- One profile for every node, whoever it is, opened from a chat, the list or the map: message, sign in, show on the map; its route in words, with flooding and the route's time limit on its own page; telemetry (Cayenne LPP decoded); rename, share on air, favourite, remove.
- A map of every node that advertises a position, and this radio: markers by kind, faded as their last advert ages; pick one for its distance and bearing, and the route the radio holds to it drawn through the relays whose position is known. OpenStreetMap tiles, kept on the device as they are seen so the map still draws where there is no network. On a phone the list rides in a sheet over the map with three positions.
- Your repeaters, rooms and sensors, managed remotely through your radio, gathered at the top of Mesh with a dot on the tab when one's battery runs low. Sign in (the password kept in the system's credential store on the desktop, the phone's secure storage on a phone, the browser's storage in a browser); status with a week's trend of battery and noise floor; the neighbours a repeater hears; its settings as forms over its console, with a timed trial before a radio change; who may sign in, and as what; the console itself; a sensor's min, max and mean over a window. The radio carries one request to a remote node at a time, so they queue, visibly, and nothing asks the air on its own.
- The radio's own settings, one page per topic, applied as they change: name, position, frequency/bandwidth/SF/CR with presets and transmit power (the one page with an Apply, since a wrong value cuts the radio off the mesh), auto-add and telemetry policy, tuning, clock, adverts (zero-hop or flood), reboot, factory reset.
- Channels are made, joined (name and 128-bit key) and left from the chats.
- System notifications for a message that arrives and for a node heard for the first time, while the window is elsewhere: native toasts on the desktop, local notifications on a phone, the Web Notification API in a browser. A click opens the chat or the contact.
